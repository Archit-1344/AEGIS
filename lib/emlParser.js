...
(function (root) {
  ...
  function scanHeaderBlock(rawEml, limits) {
    const physicalLines = [];
    let start = 0;
    let headerBytes = 0;

    for (let index = 0; index <= rawEml.length; index += 1) {
      const atEnd = index === rawEml.length;
      const isCr = rawEml[index] === "\r";
      const isLf = rawEml[index] === "\n";
      if (!atEnd && !isCr && !isLf) continue;

      const line = rawEml.slice(start, index);
      if (/^[ \t]*$/.test(line)) {
        return { physicalLines, hadHeaderBodySeparator: !atEnd };
      }
      ...
      headerBytes += BufferByteLength(line) + 2;
      if (headerBytes > limits.maxHeaderBytes) {
        throw parserError("EML_HEADERS_TOO_LARGE", "Header block exceeds the configured size limit.");
      }
      ...
      if (isCr && rawEml[index + 1] === "\n") index += 1;
      start = index + 1;
    }

    return { physicalLines, hadHeaderBodySeparator: false };
  }
  ...
  function parseEmlHeaders(rawEml, options) {
    ...
    if (options?.limits?.maxInputBytes && !options?.limits?.maxHeaderBytes) {
      limits.maxHeaderBytes = options.limits.maxInputBytes;
    }

    const split = scanHeaderBlock(rawEml, limits);
    const physicalLines = split.physicalLines;
  ...
  const api = { DEFAULT_LIMITS, normalizeLineEndings, scanHeaderBlock, parseEmlHeaders };
