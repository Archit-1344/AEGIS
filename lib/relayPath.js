...
(function (root) {
  ...
  function reconstructRelayPath(receivedValues, options) {
    ...
    const earliestWithIp = deliveryOrder.find(hop => hop.observedIps.length > 0) || null;
    return Object.freeze({
      sourceOrder: Object.freeze(sourceOrder.map(Object.freeze)),
      deliveryOrder: Object.freeze(deliveryOrder),
      earliestObservableInfrastructureIp: earliestWithIp?.observedIps[0] || null,
      earliestIpConfidence: earliestWithIp ? "low" : "unavailable",
  ...
  const api = { extractReceivedFacts, reconstructRelayPath };
